# AGENTS.md — Baby Store Management Web App

Always-on project rules. Detailed, topic-specific rules live in `.agents/skills/*/SKILL.md` —
Antigravity loads a skill's full instructions when the task matches it. This file is the
...
library "because it's slightly nicer" — extend what's already chosen (see AGENTS.md §2 in
...
2. Read the skills relevant to the phase (`.agents/skills/`).
...
See `.agents/skills/` for topic-specific, actionable rules: React/TS fundamentals,
